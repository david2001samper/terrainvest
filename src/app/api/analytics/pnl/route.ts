...
function realizedBasis(total: unknown, pnl: number): number {
  const exitValue = Number(total) || 0;
  const basis = Math.abs(exitValue - pnl);
  return basis > EPSILON ? basis : Math.abs(exitValue);
}
...
export async function GET(request: Request) {
  try {
    ...
    const [{ data: trades, error }, { data: profile, error: profileError }] = await Promise.all([
      supabase
        .from("trades")
        .select("symbol, side, quantity, price, total, profit_loss, created_at")
        .eq("user_id", user.id)
        .eq("status", "filled")
    ...
    const dailyBasisMap = new Map<string, number>();
    const symbolMap = new Map<
      string,
      { pnl: number; count: number; wins: number }
    >();
    let totalRealizedBasis = 0;

    for (const t of realizedRows) {
      const pnl = Number(t.profit_loss) || 0;
      const day = dayKeyForInstant(new Date(t.created_at), tzOffsetMinutes);
      const basis = realizedBasis(t.total, pnl);

      dailyMap.set(day, (dailyMap.get(day) || 0) + pnl);
      dailyBasisMap.set(day, (dailyBasisMap.get(day) || 0) + basis);
      totalRealizedBasis += basis;
    ...
    const todayRealizedBasis = dailyBasisMap.get(todayStr) || 0;
    ...
    const totalPnlPercent = totalRealizedBasis > EPSILON ? (totalPnl / totalRealizedBasis) * 100 : 0;
    const todayPnlPercent = todayRealizedBasis > EPSILON ? (todayPnl / todayRealizedBasis) * 100 : 0;
