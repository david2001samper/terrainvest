...
export function useMarketData() {
  ...
  // payloads actually change, otherwise consumers (memo/useEffect deps,
  // child component props) re-render on every parent render.
  const allAssets = useMemo(
    () => [...(crypto.data || []), ...(stocks.data || []), ...(forex.data || [])],
    [crypto.data, stocks.data, forex.data]
  );
